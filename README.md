...
| `app` | Mini App (React + Vite), публикуется на Cloudflare Pages |
...
1. **Бот.** В @BotFather: `/newbot`, затем `/setinline` (включить inline-режим, подсказка «сумма заказа»), затем `/newapp` или «Main Mini App» с адресом адресом сайта на Cloudflare Pages (`https://<проект>.pages.dev`).
...
3. **Секреты функций** (Supabase → Edge Functions → Secrets):
   ...
   - `APP_URL` — адресом сайта на Cloudflare Pages (`https://<проект>.pages.dev`)
   - `ADMIN_TELEGRAM_IDS` — telegram id админов через запятую
   - `BOT_USERNAME` — имя бота без @ (необязательно)
4. **Mini App.** Cloudflare → Workers & Pages → Create → Pages → Connect to Git → этот репозиторий. Настройки сборки: Root directory `app`, Build command `npm run build`, Build output `dist`. Адрес API и публичный ключ уже лежат в `app/.env.production`; имя бота задать переменной `VITE_BOT_USERNAME`.
